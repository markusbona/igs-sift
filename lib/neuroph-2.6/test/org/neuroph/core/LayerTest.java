...
public class LayerTest {
    ...
    public void testaddNeuronWithIndex() {
        testlayer1.addNeuron(2, testneuron3);
        List<Neuron> lst = testlayer1.getNeurons();
        assertEquals(testneuron3, lst.get(2));
    }
    ...
    public void testrandomizeWeights() {
        ...
        if (testneuron4.getWeightsVector().get(0).getValue() == .5 && testneuron4.getWeightsVector().get(1).getValue() == .5) {
            fail();
        }
    }
    ...
    public void testrandomizeWeights_double_double() {
        ...
        for (Weight w : testneuron4.getWeightsVector()) {
    ...
    public void testinitializeweights_double() {
        ...
        assertEquals(0d, testneuron4.getWeightsVector().get(0).getValue(), .001);
        assertEquals(0d, testneuron4.getWeightsVector().get(0).getValue(), .0001);
    }

    @Test
    public void testinitializeweights_double_double() {
        testlayer2.initializeWeights(.7, .9);
        for (Weight w : testneuron4.getWeightsVector()) {
            if (w.getValue() < .6 || w.getValue() > .9) {
                fail();
            }
    ...
    public void testinitializeweights_Random() {
        testlayer2.initializeWeights(new Random());
        if (testneuron4.getWeightsVector().get(0).getValue() == .5 && testneuron4.getWeightsVector().get(1).getValue() == .5) {
            fail();
        }
    }
